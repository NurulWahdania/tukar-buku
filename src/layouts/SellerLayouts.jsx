...
// Komponen Sidebar Dinamis untuk Seller
function SellerSidebar({ activeKey, setActiveKey, sellerData }) {
  const location = useLocation();
  const navigate = useNavigate();
  const toTarget = (p) => p;
  const [infoOpen, setInfoOpen] = useState(false);

  // jika sellerData ada -> tampilkan "Toko" (ke /seller/store)
  // jika tidak -> tampilkan "Daftarkan Toko" (ke /register-store)
  const isSeller = Boolean(sellerData);
  const menuItems = [
    // arahkan Beranda ke seller home
    { key: 'home', path: '/seller/home', label: 'Beranda' },
    { key: 'about', path: '/seller/about', label: 'Tentang' },
    { key: 'wishlist', path: '/seller/wishlist', label: 'Wishlist' },
    { key: 'store', path: '/seller/store', label: 'Toko' }
  ...
  const Icon = ({ name }) => {
    ...
    switch (name) {
      ...
      case 'Toko':
      case 'Daftarkan Toko': // gunakan ikon yang sama untuk konsistensi
        return (
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    ...
    <aside className="fixed left-8 top-28 w-60 h-[calc(100vh-220px)] flex flex-col gap-3 z-90 pointer-events-auto">
      ...
      {infoOpen && (
        <div className="pl-6 flex flex-col gap-2 mt-1">
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); setActiveKey('announcements'); navigate(toTarget('/seller/announcements')); }}
          ...
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); setActiveKey('security-tips'); navigate(toTarget('/seller/security-tips')); }}
...
// Komponen Layout Utama Seller
export default function SellerLayouts() {
  // State untuk menyimpan data seller agar bisa diupdate dan dirender ulang
  const [sellerData, setSellerData] = useState(null);
  
  const [searchParams] = useSearchParams();
  const [search, setSearch] = useState(searchParams.get('search') || ""); // State pencarian

  // Logika Pengambilan Data seller (Self-Healing)
  useEffect(() => {
    const loadSeller = async () => {
      // 1. Cek LocalStorage dulu (agar cepat)
      const authRaw = localStorage.getItem('authSeller');
      if (authRaw) {
        try {
            setSellerData(JSON.parse(authRaw));
        } catch(e) {
      ...
      if (token) {
        try {
          ...
          if (JSON.stringify(data) !== authRaw) {
              setSellerData(data);
              localStorage.setItem('authSeller', JSON.stringify(data));
          }
        } catch (err) {
  ...
  const displayName = sellerData?.nama_lengkap || sellerData?.sellername || 'Seller';
  const avatarSrc = sellerData?.profile_photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=FFE4C7&color=000000&size=64`;
  ...
  const handleSearchChange = (e) => {
    ...
    setSearch(val);
    navigate(`/seller/home?search=${encodeURIComponent(val)}`, { replace: true });
  };

  const [active, setActive] = useState('home');
  useEffect(() => {
    const p = location.pathname || '';
    // pastikan juga mendeteksi /seller/store sehingga menu "Toko" menjadi aktif saat dibuka
    if (p.includes('/seller/store') || p.includes('/register-store')) setActive('store');
    else if (p.includes('/seller/home') || p === '/seller') setActive('home');
    else if (p.includes('/wishlist')) setActive('wishlist');
    else if (p.includes('/seller/announcements') || p.includes('/announcements')) setActive('announcements');
    else if (p.includes('/seller/security-tips') || p.includes('/security-tips')) setActive('security-tips');
    else if (p.includes('/seller/about') || p.includes('/about')) setActive('about');
    else setActive('home');
  }, [location.pathname]);
  ...
  return (
    <div className="min-h-screen min-w-full relative font-inter">
          ...
          {profileOpen && (
            <div className="absolute right-0 top-full mt-2 w-60 z-60 pointer-events-auto">
              <div className="bg-white/5 rounded-2xl border border-white/10 p-3 space-y-3 mt-2 backdrop-blur-xl bg-black/80">
                ...
                <button
                  onClick={() => { setProfileOpen(false); navigate('/seller/history'); }}
                  className="w-full py-3 rounded-md text-[#FFE4C7] text-base font-medium hover:bg-white/10 text-left pl-3 flex items-center"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-3 text-[#FFE4C7]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      <SellerSidebar activeKey={active} setActiveKey={setActive} sellerData={sellerData} />
