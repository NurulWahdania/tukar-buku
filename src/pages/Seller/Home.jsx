...
function BookDetailModal({ book, onClose, isWish, onToggle, onViewReviews }) {
  ...
  const submitReport = async () => {
    ...
    } catch (error) { alert("Gagal mengirim laporan."); } finally { setSubmitting(false); }
  };
  ...
  const handleStoreClick = () => {
    if (book?.store?.id) {
        onClose(); 
        navigate(`/store-profile/${book.store.id}`); 
    }
  };
  ...
  return typeof document !== 'undefined' ? createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {ownStoreAlertOpen && ( <div className="fixed inset-0 z-[10001] flex items-center justify-center"><div className="absolute inset-0 bg-black/80" onClick={() => setOwnStoreAlertOpen(false)} /><div className="relative z-10 w-[400px] bg-[#06070a] border border-white/20 rounded-2xl p-8 text-center"><h3 className="text-[#FFE4C7] text-xl font-bold mb-2">Toko Anda Sendiri</h3><p className="text-gray-400 mb-4">Tidak bisa transaksi buku sendiri.</p><button onClick={() => setOwnStoreAlertOpen(false)} className="px-6 py-2 bg-[#FFE4C7] text-black rounded">Oke</button></div></div> )}
