...
function BookDetailModal({ book, onClose, isWish, onToggle }) {
  ...
  // State untuk menyimpan riwayat transaksi user (untuk validasi report)
  const [hasTransacted, setHasTransacted] = useState(false);
  
  // State untuk modal peringatan transaksi (pengganti alert)
      ...
      const checkTransaction = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Cek apakah ada transaksi dengan book_id yang sama
...
export default function Home() {
  ...
  const filteredBooks = books.filter(book => {
    ...
    if (selectedCategory === 'Barter') return book.is_barter === true;
    if (selectedCategory === 'Jual') return book.is_barter === false;
    
    // Filter berdasarkan nama kategori (menangani relasi Many-to-Many: categories array)
    if (Array.isArray(book.categories)) {
      return book.categories.some(cat => cat.name === selectedCategory);
    }
    // Fallback jika kategori masih single object (legacy)
    if (book.category) {
