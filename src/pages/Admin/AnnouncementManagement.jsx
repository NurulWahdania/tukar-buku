...
export default function AnnouncementManagement() {
  ...
  const handleAdd = async () => {
    ...
    try {
        ...
        if (editingId) {
            // UPDATE
            const res = await fetch(`http://localhost:8000/announcements/${editingId}`, {
                method: 'PUT',
                headers: {
  ...
  const confirmDelete = async () => {
    ...
    const token = localStorage.getItem('token') || localStorage.getItem('access_token');
    try {
        const res = await fetch(`http://localhost:8000/announcements/${deleteCandidate.id}`, {
            method: 'DELETE',
  ...
  return (
    <div className="w-[1020px] relative mx-auto rounded-[20px] overflow-hidden">
      ...
      <div className="relative p-6">
        <h2 className="text-[#FFE4C7] text-xl font-semibold mb-4">Manajemen Pengumuman</h2>
        ...
        <div className="bg-neutral-300/10 rounded-xl outline outline-1 outline-white/0 backdrop-blur-lg overflow-hidden">
          ...
          <div className="max-h-[320px] overflow-auto">
            {loading ? (
                ...
                <div className="p-4 text-[#FFE4C7] text-center opacity-60">Belum ada pengumuman.</div>
            ) : (
                items.map((it, idx) => (
      ...
      {isModalOpen && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center" role="dialog" aria-modal="true" aria-label={editingId ? 'Edit Pengumuman' : 'Buat Pengumuman'}>
          ...
          <div className="relative z-[100000] w-[1020px] h-96 rounded-xl outline outline-1 outline-offset-[-1px] outline-white/0 bg-black/60 backdrop-blur-lg p-6">
            ...
            <div className="absolute left-[20px] top-[16px] text-[#FFE4C7] text-xl font-semibold font-['Inter']">{editingId ? 'Edit Pengumuman' : 'Buat Pengumuman'}</div>
      ...
      {createPortal(
        confirmDeleteOpen ? (
          <div className="fixed inset-0 z-[999999] flex items-center justify-center" role="presentation">
            ...
            <div role="dialog" aria-modal="true" aria-labelledby="confirm-delete-announcement" className="relative z-[1000000] w-96 p-6 bg-[#06070a] border border-white/10 rounded-xl shadow-2xl text-center">
              <h3 className="text-[#FFE4C7] text-lg font-semibold mb-4">Yakin ingin menghapus?</h3>
              <p id="confirm-delete-announcement" className="text-[#CDBA9A] mb-6">Hapus pengumuman: <span className="text-white font-semibold">{deleteCandidate?.title}</span></p>
